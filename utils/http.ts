...
*/
const BAS_EURL = 'http://192.168.10.232:3000'
// const BAS_EURL = 'http://192.168.176.17:9051'
// const BAS_EURL = 'http://106.13.195.130:9030'


const fetch = (url: string, options?: any, headers?: any, baseURL?:string,body:any): Promise<any> =>{
  ...
  return  new Promise((resolve, reject) =>{
    ...
    useFetch(reqUrl,{...options,key,
      ...
      onResponse({ request, response, options }) {
        ...
        if(response?._data?.code === 200 || response?.status === 200) {
          resolve(response._data?.data || response._data)
        }else{
          reject(response._data?.message || response._data?.msg)
        }
      },
...
export default class Http {
  ...
  uploadJson(opt: HttpParms): Promise<any> {
    return fetch(opt.url, {method: 'post', params: 123}, opt.headers,opt.baseURL,opt.data)
  }
}
